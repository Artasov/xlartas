...
from apps.shop.models import Promo
...
class BuyProductProgramForm(FormWithRecaptchaValidator):
    license_type = forms.ChoiceField(choices=SoftwareSubscriptionVariant.choices, required=True)
    ...
    def clean(self):
        ...
        price = get_product_price_by_license_type(
            product_, cleaned_data.get('subscription_variant'))
        if not price:
