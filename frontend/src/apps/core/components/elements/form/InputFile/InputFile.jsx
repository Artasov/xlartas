...
import closeIcon from '../../../../static/img/icons/close.png';
...
const InputFile = ({name, maxFileSize, required, label, helpText, className, size}) => {
    ...
    return (
        <div className={className + ' form-field'}>
            <div
                className={`fccc file-dropzone ${classes.bgPrimary10} ${dragOver ? 'drag-over' : ''}`}
            ...
            >
                ...
                {helpText ? <span className={`fs-7 ${classes.textPrimary30}`}>Drag and drop or click</span> : ''}
            </div>
            <div className={`uploaded-files-container ${classes.textPrimary45} fs-7 px-1`}>
                {files.map(file => (
                    <div key={file.name} className="uploaded-file frcc">
                        ...
                        <img src={closeIcon} alt="Удалить"
                             width={12} height={12}
                             className={`${classes.textPrimary80}`}
