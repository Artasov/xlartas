...
const OrderItem: React.FC<OrderItemProps> = ({order, onClick, onSomeUpdatingOrderAction}) => {
    ...
    return (
        <FRBC p={2} g={1} rounded={3} wrap flexGrow={1}
              ...
              boxShadow={`0.06rem 0.06rem 0.37rem -0.1rem ${theme.palette.bg.contrast20}`}>
            <FC g={1} w={'100%'} color={theme.palette.text.primary70}>
                <FC onClick={onClick}>
                    <span className={`fs-7`} style={{color: theme.palette.text.primary40}}>
                        # {order.id}
                    </span>
                    <FRBC wrap mb={1}>
